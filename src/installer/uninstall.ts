...
/**
 * Remove the ARE-managed section from .npmrc.
 *
 * Finds the BEGIN/END delimited section and removes it.
 ...
 * @returns 'removed' if section was removed, 'deleted' if file was deleted, 'not-found' if no section
 */
function removeNpmrcSection(
  npmrcPath: string,
  dryRun: boolean,
  ...
  for (const line of lines) {
    if (line.trim() === NPMRC_BEGIN_MARKER) {
      inSection = true;
      changed = true;
      continue;
    }
    if (inSection && line.trim() === NPMRC_END_MARKER) {
      inSection = false;
      continue;
    }
    if (!inSection) {
  ...
  // Trim leading blank lines that may result from removal
  ...
  if (!dryRun) {
    if (remaining.length === 0) {
      ...
      writeFileSync(npmrcPath, remaining + '\n', 'utf-8');
    }
  }

  return remaining.length === 0 ? 'deleted' : 'removed';
}
...
function uninstallFilesForRuntime(
  ...
  if (runtime === 'claude' || runtime === 'gemini') {
    ...
    // Remove ARE npm cache section from .npmrc (local installs only).
    if (location === 'local') {
      const npmrcPath = path.join(path.dirname(basePath), '.npmrc');
      const npmrcResult = removeNpmrcSection(npmrcPath, dryRun);
      if (npmrcResult === 'removed' || npmrcResult === 'deleted') {
