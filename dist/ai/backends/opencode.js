 ...
 */
function calculateCostFromTokens(inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens) {
 ...
 */
function resolveModelForOpenCode(model) {
    if (model.includes('/'))
        return model;
...
export class OpenCodeBackend {
     ...
     */
    async ensureProjectConfig(projectRoot) {
        const agentDir = path.join(projectRoot, '.opencode', 'agents');
        await mkdir(agentDir, { recursive: true });
        await writeFile(path.join(agentDir, `${OPENCODE_AGENT_NAME}.md`), OPENCODE_AGENT_CONTENT, 'utf-8');
    }
    /**
    ...
    parseResponse(stdout, durationMs, exitCode) {
        ...
        if (!parsed.text) {
            throw new AIServiceError('PARSE_ERROR', `No text content found in OpenCode CLI output. ` +
        ...
        if (parsed.text.includes('MAXIMUM STEPS REACHED')) {
            ...
            if (cleaned.length < 100) {
                throw new AIServiceError('PARSE_ERROR', `OpenCode hit agent step limit — response is meta-commentary, not content. ` +
        ...
        if (cost === 0 && (parsed.inputTokens > 0 || parsed.outputTokens > 0)) {
            cost = calculateCostFromTokens(parsed.inputTokens, parsed.outputTokens, parsed.cacheReadTokens, parsed.cacheWriteTokens);
        }
        return {
            text: parsed.text,
