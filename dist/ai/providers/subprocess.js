...
export class SubprocessProvider {
    ...
    async call(options) {
        ...
        if (this.debug) {
            ...
            this.log.debug(`[debug] Spawning subprocess for "${taskLabel}" ` +
        ...
        if (result.timedOut) {
            this.log.warn(`[warn] Subprocess timed out after ${(result.durationMs / 1000).toFixed(1)}s ` +
        ...
        if (this.debug) {
            this.log.debug(`[debug] Subprocess exited for "${taskLabel}" ` +
     ...
     */
    enqueueSubprocessLog(result, taskLabel) {
        if (this.subprocessLogDir === null)
            return;
