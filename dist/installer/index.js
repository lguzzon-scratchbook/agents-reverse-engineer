 ...
 */
import { getAllRuntimes } from './paths.js';
import { displayBanner, showHelp, showSuccess, showError, showWarning, showInfo, showNextSteps, } from './banner.js';
import { selectRuntime, selectLocation, isInteractive } from './prompts.js';
import { installFiles, verifyInstallation } from './operations.js';
 ...
 */
function determineLocation(args) {
 ...
 */
function determineRuntimes(runtime) {
    if (!runtime) {
...
export async function runInstaller(args) {
    ...
    let selectedRuntime = runtimeArg;
 ...
 */
async function runInstall(runtime, location, force, quiet) {
    // Install files
 ...
 */
function runUninstall(runtime, location, quiet) {
 ...
 */
function displayUninstallResults(results, configDeleted = false) {
    console.log();
    let totalDeleted = 0;
