...
import { getClaudeTemplates, getCodexTemplates, getOpenCodeTemplates, getGeminiTemplates, } from '../integration/templates.js';
/**
...
const LOCAL_CONTEXT_RULES_MARKER = '<!-- Generated by agents-reverse-engineer installer: local-context-rules -->';
const CODEX_PREFIX_RULES = [
    ['npx', 'are', 'init'],
    ['npx', 'are', 'discover'],
    ['npx', 'are', 'generate'],
    ['npx', 'are', 'update'],
...
function getCodexRulesFilePath(location, basePath) {
...
function getCodexContextRulesFilePath(location, basePath) {
    if (location === 'local') {
...
function buildCodexRulesFileContent() {
    const lines = [
...
function buildLocalContextRulesContent() {
 ...
 */
function shouldWriteManagedFile(filePath, marker, force) {
...
function installFilesForRuntime(runtime, location, options) {
    ...
    if (runtime === 'claude' || runtime === 'gemini') {
        ...
        const shouldWrite = shouldWriteManagedFile(contextRulesPath, LOCAL_CONTEXT_RULES_MARKER, options.force);
        if (!shouldWrite) {
