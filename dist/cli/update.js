...
import { createUpdateOrchestrator, } from '../orchestration/orchestrator.js';
...
export async function updateCommand(targetPath, options) {
    ...
    try {
        ...
        if (options.trace) {
            const logDir = path.join(absolutePath, '.agents-reverse-engineer', 'subprocess-logs', new Date().toISOString().replace(/[:.]/g, '-'));
        ...
        const summary = await runner.executeUpdate(plan.fileTasks, absolutePath, config);
        // -------------------------------------------------------------------------
        ...
        if (plan.affectedDirs.length > 0) {
            ...
            for (const dir of plan.affectedDirs) {
                ...
                try {
                    ...
                    dirReporter.onDirectoryDone(dir || '.', dirDurationMs, response.inputTokens, response.outputTokens, response.model, response.cacheReadTokens, response.cacheCreationTokens);
                    dirsCompleted++;
        ...
        await orchestrator.recordRun(plan.currentCommit, summary.filesProcessed, filesSkipped);
        // -------------------------------------------------------------------------
